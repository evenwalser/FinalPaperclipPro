...
export async function POST(
  ...
  try {
    ...
    // Call the Paperclip API to accept the offer
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_PAPERCLIP_API_URL}/v4/offers/accept/${offerId}`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      ...
      console.error("Paperclip accept offer API error:", errorText);
