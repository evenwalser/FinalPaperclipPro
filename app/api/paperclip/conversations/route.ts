...
// GET handler - fetch conversations
export async function GET(request: NextRequest) {
  try {
    ...
    // Parse the user object from the query parameter
    ...
    // Build the API URL
    const apiUrl = `${process.env.NEXT_PUBLIC_PAPERCLIP_API_URL}/v4/conversations`;
    ...
    if (!response.ok) {
      ...
      console.error("Paperclip conversations API error:", errorText);
    ...
    console.error("Conversations GET route error:", error);
    return NextResponse.json(
